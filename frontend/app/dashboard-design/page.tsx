...
export default function DashboardDesignPage() {
  ...
  const panel = useMemo(() => {
    ...
    return <WorkspacePanels active={active} campaign={campaign} resumeReady resumeName="Sajan-Giri-Resume.pdf" gmailReady busy={false} message={message} onUseTemplate={(template: CampaignTemplate) => { setCampaign({ ...MOCK_CAMPAIGN, name: `${template.title} Campaign`, location: template.location, target_business_type: template.role }); setMessage("Mock campaign updated for design preview."); setActive("campaign"); }} onResumeUpload={() => setMessage("Mock resume upload clicked.")} onConnectGmail={() => setMessage("Mock Gmail connection clicked.")} onToggleCampaign={() => setCampaign((current) => ({ ...current, status: current.status === "active" ? "paused" : "active" }))} onFindJobsNow={() => setMessage("Mock job search completed.")} />;

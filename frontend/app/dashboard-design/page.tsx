...
// Mirrors the shape of the live campaign_templates rows, with the real
// artwork, so the carousel and campaign picker can be previewed.
const MOCK_TEMPLATES: CampaignTemplate[] = [
  { id: "t-support", slug: "support-worker", title: "Support Worker / NDIS", imageUrl: "/images/templates/supportworker.jpg", role: "Disability Support Worker", location: "Sydney NSW", description: "Disability support and NDIS roles.", category: "Healthcare & NDIS", queryTerms: [], includeTitleTerms: [], excludeTitleTerms: [], descriptionKeywords: [], jobTypes: [], postedWithinDays: 30 },
  { id: "t-agecare", slug: "agecare", title: "AgeCare", imageUrl: "/images/templates/agecare.jpg", role: "Aged Care Worker", location: "Sydney NSW", description: "Aged care and personal care roles.", category: "Aged Care", queryTerms: [], includeTitleTerms: [], excludeTitleTerms: [], descriptionKeywords: [], jobTypes: [], postedWithinDays: 30 },
  { id: "t-childcare", slug: "childcare", title: "Child Care", imageUrl: "/images/templates/childcare.jpg", role: "Childcare Educator", location: "Sydney NSW", description: "Childcare and early learning roles.", category: "General", queryTerms: [], includeTitleTerms: [], excludeTitleTerms: [], descriptionKeywords: [], jobTypes: [], postedWithinDays: 30 },
  { id: "t-community", slug: "caseworker-community-services", title: "Caseworker / Community Services", imageUrl: "/images/templates/communityservice.jpg", role: "Caseworker", location: "Sydney NSW", description: "Community services and social care.", category: "Community Services & Social Care", queryTerms: [], includeTitleTerms: [], excludeTitleTerms: [], descriptionKeywords: [], jobTypes: [], postedWithinDays: 30 },
  { id: "t-accounting", slug: "accounting-jobs-sydney", title: "Accounting Jobs", role: "Accounting / Finance", location: "Darwin NT", description: "Accounts and finance roles.", category: "Accounting", queryTerms: [], includeTitleTerms: [], excludeTitleTerms: [], descriptionKeywords: [], jobTypes: [], postedWithinDays: 30 },
];

const MOCK_TOP_TEMPLATE_IDS = ["t-support", "t-accounting", "t-community"];
...
export default function DashboardDesignPage() {
  ...
  const panel = useMemo(() => {
    if (active === "overview") return <OverviewDashboard campaign={campaign} resumeReady resumeName="User Resume Name Here" gmailReady approvedCount={approvedCount} passedCount={passedCount} greetingName="Sanjaya" templates={MOCK_TEMPLATES} onOpenTemplate={() => setPreviewPanel("templates")} onOpenTracker={() => setPreviewPanel("tracker")} onBrowseTemplates={() => setPreviewPanel("templates")} onUpdateResume={() => setPreviewPanel("resume")} onConnectGmail={() => setPreviewPanel("gmail")} onSetUpCampaign={() => setPreviewPanel("campaign")} />;

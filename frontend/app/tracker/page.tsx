...
function getCampaignRole(campaign?: Campaign | null) { return cleanText(campaign?.search?.target_role) || cleanText(campaign?.target_business_type) || cleanText(campaign?.name, "target opportunities"); }
...
export default function TrackerPage() {
  ...
  const dayGroups = useMemo(() => {
    ...
    logs.forEach((log) => { const key = dayKey(log.actionTime); groups.set(key, [...(groups.get(key) || []), log]); });
    return Array.from(groups.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([key, dayLogs], index) => ({ key, label: `Day ${index + 1}`, dateLabel: formatShortDate(key), total: dayLogs.length, applied: dayLogs.filter((log) => log.status === "applied" || log.status === "queued").length }));
  }, [logs]);

  const visibleLogs = useMemo(() => selectedDayKey === "all" ? logs : logs.filter((log) => dayKey(log.actionTime) === selectedDayKey), [logs, selectedDayKey]);
  ...
  const visibleDayLabel = selectedDayKey === "all" ? "All days" : dayGroups.find((group) => group.key === selectedDayKey)?.label || "Selected day";
  ...
  const jsonHref = useMemo(() => `data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify({ exported_at: new Date().toISOString(), campaign: campaign ? { id: campaign.id, name: campaign.name, role, location } : null, summary, logs }, null, 2))}`, [campaign, role, location, summary, logs]);
  const filePrefix = safeFileName(campaign?.name || role);

  useEffect(() => { setCurrentPage(1); }, [selectedDayKey]);

  async function loadSingleCampaign(supabase: ReturnType<typeof getSupabaseClient>, userId: string) {
    const { data, error } = await supabase.from("campaigns").select("id,name,target_business_type,location,search,created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  async function loadSavedJobs(supabase: ReturnType<typeof getSupabaseClient>, userId: string, campaignId?: string | null) {
    const select = "id,title,company,location,description,apply_url,source,status,user_decision,reviewed_at,created_at,campaign_id";
    if (campaignId) {
      const campaignJobs = await supabase.from("jobs").select(select).eq("user_id", userId).eq("campaign_id", campaignId).order("created_at", { ascending: false }).limit(1000);
    ...
    const allSavedJobs = await supabase.from("jobs").select(select).eq("user_id", userId).order("created_at", { ascending: false }).limit(1000);
  ...
  async function loadTracker() {
    ...
    try {
      ...
      setCampaign(loadedCampaign); setRole(getCampaignRole(loadedCampaign)); setLocation(getCampaignLocation(loadedCampaign));
  ...
  async function saveJobStatus(logId: string, nextStatus: AgentStatus) {
    const previousLogs = logs; setSavingStatusId(logId); setActionMessage(""); setLogs((current) => current.map((log) => log.id === logId ? { ...log, status: nextStatus } : log));
    try {
      const supabase = getSupabaseClient();
      const { data: userData, error: userError } = await supabase.auth.getUser();
      if (userError || !userData.user) throw new Error("Missing login session. Please sign in again.");
      const { error } = await supabase.from("jobs").update({ status: nextStatus }).eq("id", logId).eq("user_id", userData.user.id);
      if (error) throw error;
      setActionMessage(`Status updated to ${statusLabel(nextStatus)}.`);
    } catch (error) { setLogs(previousLogs); setActionMessage(getErrorMessage(error, "Could not update status.")); } finally { setSavingStatusId(null); }
  }

  async function reviewJob(log: AgentLog, decision: "approved" | "declined") {
    const previousLogs = logs;
    const nextStatus: AgentStatus = decision === "approved" ? "approved" : "declined";
    setSavingStatusId(log.id); setActionMessage(""); setLogs((current) => current.map((row) => row.id === log.id ? { ...row, status: nextStatus } : row));
    try {
      const supabase = getSupabaseClient();
      const { data: userData, error: userError } = await supabase.auth.getUser();
      if (userError || !userData.user) throw new Error("Missing login session. Please sign in again.");
  ...
  return (
    <main className="agent-log-shell">
      <header className="agent-log-header"><Link href="/dashboard" className="agent-log-brand"><img src="/applix-logo.svg" alt="Applix logo" /><div><strong>APPLIX</strong><span>Applix Logs</span></div></Link><Link className="agent-home-link" href="/dashboard">Home</Link></header>
      <section className="agent-log-hero"><p className="agent-kicker">Track Applix Log</p><h1>See what Applix is doing.</h1><p>Approve jobs to queue them for Applix automatic sending, or decline them to stop outreach.</p></section>
      <section className="agent-log-card campaign-overview"><span className="active-pill">Active campaign</span><h2>{campaign?.name || role}</h2><p>{role} in {location}</p></section>
      <section className="agent-summary-grid"><div className="summary-card found"><span>Found</span><strong>{summary.found}</strong></div><div className="summary-card prepared"><span>Prepared</span><strong>{summary.prepared}</strong></div><div className="summary-card applied"><span>Queued</span><strong>{summary.applied}</strong></div><div className="summary-card skipped"><span>Declined</span><strong>{summary.skipped}</strong></div></section>

      {logs.length > 0 && <section className="agent-log-card agent-day-timeline"><div className="day-line-header"><div><p className="agent-kicker">Daily timeline</p><h2>Choose the day you want to inspect</h2></div><span>{visibleDayLabel} · {visibleLogs.length} jobs</span></div><div className="day-dot-row"><button className={selectedDayKey === "all" ? "active" : ""} type="button" onClick={() => setSelectedDayKey("all")}><span className="day-dot" /><strong>All</strong><small>{logs.length} jobs</small></button>{dayGroups.map((group) => <button key={group.key} className={selectedDayKey === group.key ? "active" : ""} type="button" onClick={() => setSelectedDayKey(group.key)}><span className="day-dot" /><strong>{group.label}</strong><small>{group.applied} queued · {group.total} total</small><em>{group.dateLabel}</em></button>)}</div></section>}

      <section className="agent-action-row"><button type="button" onClick={reloadLogs} disabled={loading || reloading}>{reloading ? "Reloading..." : "Reload logs"}</button><a href={csvHref} download={`${filePrefix}-applix-logs.csv`} aria-disabled={loading || logs.length === 0}>Download Excel</a><a href={jsonHref} download={`${filePrefix}-applix-logs.json`} aria-disabled={loading || logs.length === 0}>Download JSON</a></section>

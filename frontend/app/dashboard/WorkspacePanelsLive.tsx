...
export default function WorkspacePanelsLive(props: Props) {
  ...
  if (props.active === "overview") return <OverviewDashboard campaign={props.campaign} purchasedTemplate={props.purchasedTemplate} resumeReady={props.resumeReady} resumeName={props.resumeName} gmailReady={props.gmailReady} approvedCount={props.approvedCount} passedCount={props.passedCount} onOpenTracker={props.onOpenTracker} />;
  ...
  return (
    <section className={`template-browser-canva${selected ? " is-reviewing" : ""}`}>
      <header className="template-browser-head"><p>Templates</p><h1>{selected ? "Template details" : "Browse templates"}</h1><span>{selected ? "Review the campaign, choose your postcode, and confirm the price before checkout." : "Login and browsing are free. Pricing appears only after you choose a template."}</span></header>
      {selected ? (
        <div className="template-review-canva">
          ...
          <div className="template-review-hero"><div className="template-review-copy"><p className="template-category">{selected.category}</p><h2>{selected.title}</h2><p className="template-review-description">{selected.description}</p><div className="template-review-usage"><strong>Ready-made campaign</strong><span>{selected.campaignName || selected.title}</span></div></div>{selected.imageUrl ? <img src={selected.imageUrl} alt={selected.title} /> : <div className="template-review-image-placeholder">Add a template photo from Admin</div>}</div>
          <section className="template-recipe-card"><div><span>Search recipe</span><strong>Every Day Job Portal Search</strong></div><div><span>Target role</span><strong>{selected.role}</strong></div><div><span>Location</span><strong>{postcodeInfo.valid ? postcodeInfo.label : "Choose postcode below"}</strong></div><div><span>Posted within</span><strong>{selected.postedWithinDays} days</strong></div></section>
          <section style={{ border: "1px solid #ddd", padding: 18, marginTop: 18 }}><h3 style={{ marginTop: 0 }}>Choose campaign location</h3><label htmlFor="campaign-postcode" style={{ display: "grid", gap: 7, maxWidth: 420 }}><strong>Australian postcode</strong><input id="campaign-postcode" inputMode="numeric" autoComplete="postal-code" maxLength={4} value={postcode} disabled={purchased} onBlur={() => setPostcodeTouched(true)} onChange={(event) => { const nextPostcode = normaliseAustralianPostcode(event.target.value.trim()); setPostcode(nextPostcode); setCheckoutError(""); persistTemplateIntent(selected, nextPostcode); }} placeholder="Example: 2141" aria-invalid={postcodeTouched && !postcodeInfo.valid} aria-describedby="campaign-postcode-message" style={{ minHeight: 48, border: "1px solid #bbb", padding: "0 14px", fontSize: 16, fontWeight: 800 }} /></label><div id="campaign-postcode-message" aria-live="polite">{postcodeTouched && !postcodeInfo.valid ? <p style={{ color: "#a12a38", fontWeight: 800, marginBottom: 0 }}>Enter a valid 4-digit Australian postcode.</p> : null}{postcodeInfo.valid ? <p style={{ color: "#226d35", fontWeight: 900, marginBottom: 0 }}>Detected location: {postcodeInfo.label}</p> : null}</div><p style={{ color: "#666", lineHeight: 1.5, marginBottom: 0 }}>Your postcode will be used to rank nearby jobs and providers that service your area.</p></section>
          <section style={{ border: "1px solid #ddd", padding: 18, marginTop: 18 }}><h3 style={{ marginTop: 0 }}>Included with this template</h3><div style={{ display: "grid", gap: 9 }}>{(selected.pricingFeatures || []).map((feature) => <div key={feature}>✓ {feature}</div>)}</div></section>
          {purchaseState === "processing" ? <div className="workspace-message" role="status" style={{ marginTop: 16 }}>Checkout is still being confirmed.</div> : null}
          {purchaseState === "purchased_campaign_missing" ? <div className="workspace-message" role="status" style={{ marginTop: 16 }}>Checkout completed. No additional payment is required.</div> : null}
          {purchaseState === "purchased_campaign_ready" ? <div className="workspace-message" role="status" style={{ marginTop: 16 }}>This campaign is already in your account.</div> : null}
          {purchaseState === "failed_or_expired" ? <div className="workspace-message" role="alert" style={{ marginTop: 16 }}>The previous checkout failed or expired. Your draft is safe and you can try again.</div> : null}
          {checkoutError ? <div className="workspace-message" role="alert" style={{ marginTop: 16 }}>{checkoutError}</div> : null}
          <div className="template-review-checkout"><div><span>Campaign template</span><strong>{selected.campaignName || selected.title}</strong><small>{postcodeInfo.valid ? postcodeInfo.label : "Enter postcode to continue"}</small></div><section aria-label="Template price" style={{ display: "flex", alignItems: "center", justifyContent: "center", flexWrap: "wrap", gap: 12, marginLeft: "auto", whiteSpace: "nowrap" }}>{selected.compareAtPriceAmount ? <div style={{ color: "#ff3f4f", fontSize: 24, fontWeight: 900, textDecoration: "line-through" }}>{formatMoney(selected.compareAtPriceAmount, selected.currency)}</div> : null}<div style={{ color: "#2f8f2f", fontSize: 30, fontWeight: 950 }}>{formatMoney(selected.priceAmount || 0, selected.currency)}</div><div style={{ color: "#555", fontSize: 14, fontWeight: 700 }}>{selected.priceLabel}</div></section><button type="button" className="workspace-primary" onClick={() => void buttonAction()} disabled={purchaseLoading || checkoutNavigating || (!postcodeInfo.valid && purchaseState === "not_purchased")} style={{ position: "relative", zIndex: 2, pointerEvents: checkoutNavigating ? "none" : "auto" }}>{buttonLabel}</button></div>
        </div>
      ) : <><label className="template-search-label" htmlFor="template-search">Search templates or job roles</label><input id="template-search" className="workspace-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search templates or job roles" />{loading && <div className="workspace-message" role="status" aria-live="polite">Loading templates…</div>}{error && <div className="workspace-message" role="alert">{error}</div>}<div className="template-canva-grid">{visible.map((item) => <article className="template-canva-card" key={item.id}><span className="template-category">{item.category}</span><h3>{item.title}</h3>{item.imageUrl ? <img className="template-canva-image" src={item.imageUrl} alt={item.title} /> : <div className="template-canva-image template-canva-placeholder">Add a photo from Admin</div>}<p>{item.description}</p><small>{item.role} · Choose postcode</small><button type="button" onClick={() => openTemplate(item)}>Review template →</button></article>)}</div>{!loading && !error && visible.length === 0 && <div className="workspace-message">No active templates match this search.</div>}</>}
    </section>
  );
}

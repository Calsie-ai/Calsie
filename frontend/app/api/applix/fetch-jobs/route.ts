...
function getDetailsSummary(value: unknown) {
  ...
  if (typeof value === "object") {
    ...
    const message = record.error || record.message || record.detail || record.details || record.provider_status_text;
    return message ? String(message).slice(0, 300) : JSON.stringify(value).slice(0, 300);
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (!response.ok || !data?.ok) {
      const details = getDetailsSummary(data?.details || data?.attempts || data);
      const error = details ? `${data?.error || "OutScraper job fetch failed."} ${details}` : data?.error || "OutScraper Edge Function could not fetch jobs.";
      return NextResponse.json({ ok: false, error, details: data }, { status: response.status || 500 });
    }

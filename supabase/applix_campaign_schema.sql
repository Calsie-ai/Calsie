...
-- 2. Search batches generated by the daily Render scraper.
-- These turn a target role into concrete business discovery queries.
create table if not exists campaign_search_batches (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists campaign_search_batches_campaign_id_idx on campaign_search_batches(campaign_id);
create index if not exists campaign_search_batches_status_idx on campaign_search_batches(status);
create index if not exists campaign_search_batches_type_idx on campaign_search_batches(batch_type);
create unique index if not exists campaign_search_batches_unique_query
on campaign_search_batches(campaign_id, search_query);

-- 3. Raw company/job/company-opportunity leads inserted by Python scraper.
create table if not exists company_leads (
  id uuid primary key default gen_random_uuid(),
  campaign_id uuid references campaigns(id) on delete cascade,
  search_batch_id uuid references campaign_search_batches(id) on delete set null,
  user_id uuid,
...
create index if not exists company_leads_search_batch_id_idx on company_leads(search_batch_id);
...
-- 4. Enrichment details created by n8n/OpenAI after a lead is scraped.
...
-- 5. AI-generated outreach/resume drafts before sending.
...
-- 6. Email queue for later stage. Not used by the current scraper-first MVP.
...
-- 7. Daily quota tracking for 25/day rule.
...
-- 8. Report table for dashboard summary.
...
-- 9. Event log for debugging and dashboard timeline.
...
-- 10. Helper function to refresh campaign report counts.
...
begin
  ...
  select
    ...
    (select count(*) from campaign_search_batches b where b.campaign_id = c.id),
    (select count(*) from campaign_search_batches b where b.campaign_id = c.id and b.status = 'processed'),
  ...
  do update set
    ...
    total_batches_processed = excluded.total_batches_processed,
...
drop trigger if exists set_campaign_search_batches_updated_at on campaign_search_batches;
create trigger set_campaign_search_batches_updated_at before update on campaign_search_batches for each row execute function set_updated_at();

drop trigger if exists set_company_leads_updated_at on company_leads;
create trigger set_company_leads_updated_at before update on company_leads for each row execute function set_updated_at();

drop trigger if exists set_lead_enrichments_updated_at on lead_enrichments;
create trigger set_lead_enrichments_updated_at before update on lead_enrichments for each row execute function set_updated_at();

drop trigger if exists set_outreach_drafts_updated_at on outreach_drafts;
create trigger set_outreach_drafts_updated_at before update on outreach_drafts for each row execute function set_updated_at();

drop trigger if exists set_outreach_queue_updated_at on outreach_queue;
create trigger set_outreach_queue_updated_at before update on outreach_queue for each row execute function set_updated_at();

drop trigger if exists set_campaign_daily_usage_updated_at on campaign_daily_usage;
create trigger set_campaign_daily_usage_updated_at before update on campaign_daily_usage for each row execute function set_updated_at();

drop trigger if exists set_campaign_reports_updated_at on campaign_reports;
create trigger set_campaign_reports_updated_at before update on campaign_reports for each row execute function set_updated_at();
